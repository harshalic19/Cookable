...
struct Meal: Codable {
    ...
    // Ingredients and measures are numbered
    ...
    func combinedIngredients() -> [String] {
        let ingredients = [
        ...
        return ingredients.compactMap { ing, meas in
            guard let ing = ing?.trimmingCharacters(in: .whitespacesAndNewlines), !ing.isEmpty else { return nil }
            let m = (meas ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
            return m.isEmpty ? ing : "\(ing) - \(m)"
        }
    }
}
